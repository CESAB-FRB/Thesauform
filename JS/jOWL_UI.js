...
*/
	owl_autocomplete : function(options){
		options = $.extend({
			...
			//alert(item.label());
			//alert();
			$("#pere").html(item.label());
			$("#Apere").html(item.label());
			$("#pereadd").html(item.label());
			var html = $.ajax({ url: "info.jsp?trait="+item.label(), async: false }).responseText;
			//alert(html);
			$("#hpere").val(item.label());
			$("#inputAdd").val(item.label());
			$("#inputAnn").val(item.label());
			$("#bodygrid").html(html);
			
			$("#jtree").jstree("search",item.label());
			...
			formatListItem : function(listitem, type, identifier, termarray){ //formatting of results, can be overridden
				...
				if(termarray.length) { listitem.append($('<div class="terms"/>').text(termarray[0])
					.prepend($('<span/>').addClass('termlabel').text("Terms: ")));
			}
		}}, options);
		jOWL.UI.asBroadcaster(this);
	 ...
	 * TEST
	 */
	owl_autocompleteA : function(options){
		options = $.extend({
			time:500, //responsetime to check for new keystrokes, default 500
			chars:3, //number of characters needed before autocomplete starts searching
			focus:false, //put cursor on the input field when loading
		...
		results.populate = function(data){
			...
			for(x in data){
				if(count < options.limit){
					...
					;
					var list = $('<li/>').data("jowltype", x)
					.click(function(){options.champ.val(x.split('#')[1]);}).hover(onHover, offHover)
					.appendTo(res);
					var terms = [];
					for(var l = 0;l<item.length;l++){ 
						var found = false; var newterm = item[l].term;
		...
		options = $.extend({
			...
			onSelect : function(item){	
				//alert(item.label());
					...
					//$("#ajoutSyn").clone().insertAfter("#ajoutSyn");
					
			}, //function that can be overwritten to specfy behavior when something is selected
